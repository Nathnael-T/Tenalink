...
TenaLink is a multi-portal healthcare platform that connects patients, doctors, hospital administrators, and super administrators through a unified backend. It supports medical record timelines, prescriptions, hospital discovery, and platform administration.

## Problem It Solves

The platform addresses fragmented healthcare coordination by providing:

- A single patient portal for medical history and profile management
- A doctor workspace for patient care, prescriptions, and medical events
...
|------|-------------|-------------|
| **Patient** | `/patient/*` | Views medical history, manages profile, and interacts with care records |
| **Doctor (Provider)** | `/doctor/*` | Manages patients, prescriptions, medical events, and care workflows |
| **Hospital Admin** | `/admin/*` | Oversees doctors, patients, audit logs, and settings |
...
- Care-event and record management
...
| `user_db` | user-service (+ JDBC from auth, care workflow, pharmacy, medical-records) | Patients and doctors |
| `legacy placeholder_db` | legacy placeholder | Retired care workflow dataset |
...
Spring Data JPA (Hibernate) is used across all domain services. Raw JDBC (`JdbcTemplate`) is used for cross-database reads in auth, pharmacy, and medical-records services.
